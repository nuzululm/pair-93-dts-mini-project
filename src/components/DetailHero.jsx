...
const DetailHero = ({ movieID, type }) => {
  const [movie, setMovie] = useState([]);

  useEffect(() => {
    axios.get(`${BASE_URL}/${type}/${movieID}?&api_key=${API_KEY}`).then((response) => {
    ...
    <>
      <div className="w-full h-[550px] text-white">
        <div className="w-full h-full">
          ...
          <div className="absolute top-[20%] p-4 md:p-8">
            <h1 className="text-3xl md:text-5xl font-bold">{movie.title ? movie.title : movie.name}</h1>
      ...
      <div className="w-full">
        <div className="px-5 md:px-20 py-10">
          ...
          <p className="text-md text-gray-200 font-regular">{movie.overview ? movie.overview : "tidak ada deskripsi"}</p>
        </div>
      </div>
    </>
  );
